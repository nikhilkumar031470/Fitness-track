...
export default function NutritionAnalysis() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      avg: Math.round(intakes.reduce((a,b)=>a+b,0)/(intakes.length||1))
    };
  ...
  const coreMetrics = useMemo(() => {
    ...
    const totalProtein = nutritionData.reduce(
      (sum,meal)=> sum + Number(meal.protein || 0),0);

    const totalCarbs = nutritionData.reduce(
      (sum,meal)=> sum + Number(meal.carbs || 0),0);
    ...
    const hydrationLevel = Math.min(100,
      Math.round(nutritionData.length*10));
