  ...
  const weekDates = useMemo(() => {
    ...
    return Array.from({ length: 7 }, (_, i) => {
      ...
      return {
        ...
        dayLabel: days[i],
        completion:
          d < today ? Math.floor(Math.random() * (100 - 60 + 1)) + 60 : 0,
    ...
    <section className="p-8 bg-[#0B0F1A] border border-slate-800/60 rounded-[3rem] shadow-2xl relative group overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-black text-xl text-white tracking-tighter italic uppercase leading-none">
              Weekly Pulse
            </h3>
        ...
        <LayoutGroup>
          {weekDates.map((item, i) => (
            <div key={i} className="flex flex-col items-center flex-1 min-w-0">
              <span
                className={`text-[10px] font-black mb-4 uppercase tracking-tighter italic transition-colors
              ...
              >
                ...
                <span className="text-lg font-black tabular-nums tracking-tighter relative z-10">
                  {item.date}
                </span>
