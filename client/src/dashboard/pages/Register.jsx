...
const Register = () => {
  ...
  // ADDED: profile picture required check
  if (!profilePic) {
  ...
  if (!nameRegex.test(formData.fullName)) {
    alert(
      "Full Name must be at least 3 letters and contain only alphabets and spaces",
    );
    setLoading(false);
    return;
  }
    ...
    if (profilePic && profilePic.size > 4 * 1024 * 1024) {
      alert("Profile picture size cannot exceed 2MB");
      setLoading(false);
      return;
    }
      ...
      <div className="hidden lg:flex w-5/12 bg-[#0B0F1A] border-r border-slate-800/50 flex-col justify-center p-20 relative overflow-hidden">
        ...
        >
          ...
          <motion.div
            ...
            className="p-8 bg-slate-900/50 rounded-[2.5rem] border border-white/5 backdrop-blur-xl relative group shadow-2xl"
          >
            <Star
              className="absolute -top-3 -right-3 text-sky-500 fill-sky-500 animate-pulse"
              size={24}
            />
            <p className="text-slate-400 italic text-base leading-relaxed mb-6">
              "This platform shifted my entire perspective on training. The data
              visualization is unmatched in the industry. It's not just an app;
              it's a digital coach."
            </p>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sky-500/60 font-bold text-[9px] uppercase tracking-tighter">
      ...
      <div className="w-full lg:w-7/12 flex flex-col items-center justify-center p-8 md:p-20 relative">
        ...
        >
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            <div className="flex flex-col items-center mb-4">
              <label className="relative cursor-pointer group">
                <div className="w-24 h-24 rounded-full bg-slate-800 flex items-center justify-center overflow-hidden border-2 border-slate-700 group-hover:border-sky-500 transition-all required:*">
                  {profilePic ? (
                    <img
            ...
            <div className="relative group">
              <User
                className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-600 group-focus-within:text-sky-400 transition-colors"
                size={20}
              />
              <input
                type="text"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-sky-500 text-slate-950 font-black p-5 rounded-2xl flex items-center justify-center gap-3 mt-8"
            >
              {loading ? (
          ...
          <p className="text-slate-500 text-center mt-12 text-[11px] font-black uppercase tracking-[0.2em]">
            ...
            <Link
              to="/login"
              className="text-white hover:text-sky-400 underline ml-2"
            >
              Access Vault
            </Link>
          </p>
...
const FeatureItem = ({ text, delay }) => (
  ...
  >
    ...
    <span className="text-slate-400 group-hover:text-white font-bold uppercase text-[11px] tracking-widest transition-colors">
      {text}
    </span>
  </motion.div>
);
