  ...
  const fetchWorkouts = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3000/api/fetch-workouts",
      );
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="space-y-4">
          <h1 className="text-5xl md:text-7xl font-bold tracking-tighter text-white uppercase">
            Workout <span className="text-sky-500">Vault.</span>
          </h1>
          <div className="relative group max-w-md">
            ...
            <input
              ...
              className="w-full rounded-2xl bg-slate-900 border border-slate-800 focus:border-sky-500/30 focus:bg-slate-900/80 px-12 py-3 text-sm text-white placeholder:text-slate-600 outline-none transition-all duration-300"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div
              key={i}
              className={`${s.bg} p-4 rounded-3xl flex items-center gap-4 shadow-sm`}
            >
          ...
          {filteredWorkouts.map((w) => (
            ...
            >
              ...
              <div className="p-8 pt-4 space-y-6">
                ...
                <p className="text-slate-400 text-xs">{w.Note}</p>
