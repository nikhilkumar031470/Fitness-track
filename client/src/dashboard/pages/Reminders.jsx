...
const ReminderCard = ({ reminder }) => {
  ...
  const progress = calculateProgress(reminder.date, reminder.time);
  ...
  return (
    ...
    >
      <div
        className={`absolute -right-10 -top-10 h-32 w-32 rounded-full blur-3xl opacity-0 group-hover:opacity-10 transition-opacity duration-700 ${theme.bg}`}
      />

      <div className="flex justify-between items-start mb-5 relative z-10">
        <div className="flex items-center gap-3">
          <div
            className={`p-3 rounded-xl transition-all duration-500 ${theme.light} ${theme.text} group-hover:scale-110 group-hover:rotate-3`}
          >
            <Zap size={20} />
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5">
              <div
                className={`h-1 w-1 rounded-full animate-pulse ${theme.bg}`}
              />
      ...
      <div className="flex items-center gap-4 mb-2 relative z-10 text-[10px] font-black uppercase tracking-tighter">
        ...
        <div className="flex items-center gap-1 text-gray-500 bg-gray-50 dark:bg-white/5 px-2 py-0.5 rounded-md">
          <Clock size={12} className={theme.text} /> {reminder.time}
        </div>
      </div>
...
export default function RemindersPage() {
  ...
  const fetchReminders = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        "http://localhost:3000/api/fetch-reminder"
      );

      // ✅ Filter by logged user
      const userReminders = response.data.reminderdata.filter(
        (r) => r.userID === loggedUser._id
      );
  ...
  const filteredReminders = reminders.filter((r) => {
    ...
    const progress = calculateProgress(r.date, r.time);
